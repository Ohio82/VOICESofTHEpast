...
const App: React.FC = () => {
  ...
  const handleSubmit = async (location: string, date: string, generateImages: boolean, avatarStyle: string) => {
      ...
      const imagesPromise = (async () => {
         ...
         const updatedChars = await Promise.all(scenarioData.characters.map(async (char) => {
             if (char.visualDescription) {
                 const url = await generateCharacterAvatar(char.visualDescription, scenarioData.context, avatarStyle);
