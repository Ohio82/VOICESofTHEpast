...
import { PlayIcon, PauseIcon, RotateCcwIcon, ExternalLinkIcon, InfoIcon, UserIcon, EyeIcon, EyeOffIcon, ChevronDownIcon, MessageSquareIcon, GitForkIcon, PackageIcon, BookOpenIcon, ShieldCheckIcon } from 'lucide-react';
import { HistoricalScenario, DialogueLine, Annotation, Character } from '../types';
...
import { LinguisticsModal } from './LinguisticsModal';
import { SourceAnalysisModal } from './SourceAnalysisModal';

interface ScenarioDisplayProps {
  scenario: HistoricalScenario;
  audioBuffer: AudioBuffer | null;
  onScenarioChange?: (newScenario: HistoricalScenario) => void;
}

export const ScenarioDisplay: React.FC<ScenarioDisplayProps> = ({ scenario, audioBuffer, onScenarioChange }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [showAnnotations, setShowAnnotations] = useState(false);
  const [showSources, setShowSources] = useState(false);

  // Modal states for Top 10 Intelligent Interactive Features
  ...
  const [showLinguisticsModal, setShowLinguisticsModal] = useState(false);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      {/* Top 10 Intelligent Interactive Features Toolbar */}
      <div className="bg-white/90 dark:bg-slate-800/95 border border-stone-200 dark:border-slate-700 rounded-2xl p-4 shadow-lg backdrop-blur-md flex flex-wrap gap-3 items-center justify-center">
        <button
          onClick={() => setShowBranchingModal(true)}
          className="flex items-center gap-2 text-xs font-medium bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 border border-amber-500/30 hover:bg-amber-100 dark:hover:bg-amber-900/40 px-3.5 py-2 rounded-xl transition-all shadow-sm"
        >
          <GitForkIcon size={15} /> Branch "What-If" Scenario
        </button>

        <button
          onClick={() => setShowArtifactsModal(true)}
          className="flex items-center gap-2 text-xs font-medium bg-stone-100 dark:bg-slate-700/60 text-stone-700 dark:text-slate-200 border border-stone-300 dark:border-slate-600 hover:bg-stone-200 dark:hover:bg-slate-700 px-3.5 py-2 rounded-xl transition-all shadow-sm"
        >
          <PackageIcon size={15} /> Era Artifacts & Relics
        </button>

        <button
          onClick={() => setShowLinguisticsModal(true)}
          className="flex items-center gap-2 text-xs font-medium bg-stone-100 dark:bg-slate-700/60 text-stone-700 dark:text-slate-200 border border-stone-300 dark:border-slate-600 hover:bg-stone-200 dark:hover:bg-slate-700 px-3.5 py-2 rounded-xl transition-all shadow-sm"
        >
          <BookOpenIcon size={15} /> Dialect & Etymology
        </button>
        ...
        <button
          ...
          className="flex items-center gap-2 text-xs font-medium bg-stone-100 dark:bg-slate-700/60 text-stone-700 dark:text-slate-200 border border-stone-300 dark:border-slate-600 hover:bg-stone-200 dark:hover:bg-slate-700 px-3.5 py-2 rounded-xl transition-all shadow-sm"
        >
          <ShieldCheckIcon size={15} /> Source Credibility
        </button>
      </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-stone-200 dark:border-slate-700/50 pt-8 transition-colors">
           {scenario.characters.map((char, idx) => (
             <div key={idx} className="flex flex-col items-center group">
                ...
                <div className="text-center w-full px-2 space-y-3">
                    <h4 className="font-serif-display text-xl md:text-2xl text-amber-800 dark:text-amber-200 transition-colors">{char.name}</h4>
                    {char.bio && (
                    ...
                    <button
                        onClick={() => setActiveChatCharacter(char)}
                        className="inline-flex items-center gap-2 bg-amber-600 hover:bg-amber-500 text-white text-xs font-semibold px-4 py-2 rounded-xl shadow-md transition-all hover:scale-105"
                    >
                        <MessageSquareIcon size={14} /> Chat with {char.name}
                    </button>
                </div>
             </div>
           ))}
      ...
      {/* Audio Player Card */}
      <div className="bg-gradient-to-br from-amber-100/70 to-stone-200/70 dark:from-amber-900/30 dark:to-slate-900/60 border-2 border-amber-500/50 rounded-2xl p-6 flex flex-col items-center justify-center space-y-6 backdrop-blur-md shadow-xl transition-colors duration-300">
        <div className="text-center space-y-1">
            <h4 className="font-serif-display font-bold text-lg text-amber-900 dark:text-amber-100">Historical Voice Synthesis</h4>
            <p className="text-xs text-stone-600 dark:text-slate-400">
              {audioBuffer ? 'Audio generated successfully. Click play to listen to the dialogue.' : 'Generating audio voices...'}
            </p>
        </div>
        ...
        <div className="flex items-center gap-6">
            ...
            <button
              ...
              className={`w-24 h-24 rounded-full bg-amber-600 hover:bg-amber-500 text-white shadow-xl shadow-amber-500/40 flex items-center justify-center transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed ${
                audioBuffer && !isPlaying ? 'animate-bounce ring-4 ring-amber-400/50' : ''
              }`}
              title={isPlaying ? "Pause Audio" : "Play Historical Audio"}
            >
              {isPlaying ? <PauseIcon size={36} fill="currentColor" /> : <PlayIcon size={36} fill="currentColor" className="ml-1" />}
            </button>
        </div>

        <div className="flex items-center gap-2">
            <span className={`w-2.5 h-2.5 rounded-full ${isPlaying ? 'bg-green-500 animate-ping' : audioBuffer ? 'bg-amber-500' : 'bg-stone-400'}`}></span>
            <span className="text-xs font-mono uppercase tracking-widest text-amber-900/80 dark:text-amber-200/80 font-semibold">
                {isPlaying ? 'Playing Voices...' : audioBuffer ? 'Ready to Play' : 'Loading Audio...'}
            </span>
        </div>
      </div>
      ...
      {/* Modals for Top 10 Intelligent Interactive Features */}
      {activeChatCharacter && (
        <CharacterChatModal
      ...
      {showLinguisticsModal && (
        <LinguisticsModal
          scenario={scenario}
          onClose={() => setShowLinguisticsModal(false)}
        />
      )}
      ...
      {showSourceAnalysisModal && (
        <SourceAnalysisModal
          scenario={scenario}
          onClose={() => setShowSourceAnalysisModal(false)}
        />
      )}
