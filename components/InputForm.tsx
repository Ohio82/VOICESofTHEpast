...
interface InputFormProps {
  onSubmit: (location: string, date: string, generateImages: boolean, avatarStyle: string) => void;
  isLoading: boolean;
}
...
const ARTISTIC_STYLES = ['Photo-real', 'Oil Painting', 'Sketch', 'Watercolor', 'Classic Woodcut'];
...
export const InputForm: React.FC<InputFormProps> = ({ onSubmit, isLoading }) => {
  ...
  const [avatarStyle, setAvatarStyle] = useState<string>('Photo-real');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit(location, date, generateImages, avatarStyle);
  };
    ...
    <div className="w-full max-w-md mx-auto space-y-8">
      <form onSubmit={handleSubmit} className="space-y-6 bg-white/80 dark:bg-slate-800/50 p-8 rounded-2xl border border-stone-200 dark:border-slate-700 shadow-xl backdrop-blur-sm transition-colors duration-300">
        <div className="space-y-4">
          ...
          {generateImages && (
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-stone-500 dark:text-slate-400 mb-2 transition-colors">Artistic Portrait Style</label>
                  ...
                  <button
                    ...
                    className={`py-2 px-3 rounded-lg text-xs font-medium border transition-all text-left flex items-center justify-between ${
                      avatarStyle === style 
                        ...
                        : 'bg-white dark:bg-slate-900 border-stone-300 dark:border-slate-700 text-stone-700 dark:text-slate-300 hover:border-amber-500'
                    }`}
                  >
                    <span>{style}</span>
                    {avatarStyle === style && <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></span>}
                  </button>
                ))}
              </div>
            </div>
