...
// Using Gemini 2.5 Flash for research
...
export async function generateDialogueAudio(scenario: HistoricalScenario): Promise<AudioBuffer> {
  ...
  const performGeneration = async (useTranslation: boolean): Promise<AudioBuffer> => {
      ...
      scenario.script.forEach(line => {
        ...
        const cleanText = (textContent ? String(textContent) : '').replace(/\*[^*]+\*/g, '').trim();
...
export async function generateCharacterAvatar(description: string, context: string, style: string = 'Photo-real'): Promise<string | null> {
  try {
    let stylePromptInstruction = "photorealistic, historically accurate headshot portrait";
    if (style === 'Oil Painting') {
      stylePromptInstruction = "masterpiece classical oil painting portrait, rich textures, museum lighting";
    } else if (style === 'Sketch') {
      stylePromptInstruction = "fine detailed charcoal and pencil sketch portrait on aged parchment";
    } else if (style === 'Watercolor') {
      stylePromptInstruction = "delicate watercolor and ink portrait illustration";
    } else if (style === 'Classic Woodcut') {
      stylePromptInstruction = "16th-century woodcut engraving style portrait";
    }

    const prompt = `Generate a ${stylePromptInstruction} of a person matching this description: "${description}".
    Context for clothing and era style: ${context}.
    The image should be a close-up character portrait with authentic historical details.`;
...
export async function chatWithCharacterResponse(
  characterName: string,
  userMessage: string,
  scenario: HistoricalScenario
): Promise<{ text: string; translation: string }> {
  const character = scenario.characters.find(c => c.name === characterName) || scenario.characters[0];
  const prompt = `
    You are roleplaying as ${character.name} in this historical context:
    Context: "${scenario.context}"
    Character Bio: "${character.bio || character.visualDescription}"
    
    The user (a traveler from the future) asks you: "${userMessage}"
    
    Respond authentically in character, speaking with historical worldview and tone. 
    Write your response in the historical native language or period-appropriate vernacular, and provide an English translation.
    
    Output strictly valid JSON (no markdown code blocks) with this structure:
    {
      "text": "Response in native language/period tone",
...
export async function generateBranchingScenario(scenario: HistoricalScenario, whatIfQuery: string): Promise<HistoricalScenario> {
  const prompt = `
    Based on the historical scenario:
    Context: "${scenario.context}"
    
    User "What-If" alteration: "${whatIfQuery}"
    
    Generate an alternative HistoricalScenario JSON object reflecting this counterfactual branch (same structure as original: context, accentProfile, characters, script, sources).
...
export async function fetchHistoricalArtifacts(scenario: HistoricalScenario): Promise<any[]> {
  const prompt = `
    Based on this historical context: "${scenario.context}"
    Generate 3 authentic historical artifacts or relics likely present in this place and time.
    Output strictly valid JSON array of objects with keys: "name", "origin", "description", "historicalSignificance".
    No markdown blocks.
  `;
...
export async function fetchLinguisticAnalysis(scenario: HistoricalScenario): Promise<any> {
  const prompt = `
    Analyze the language, dialect, and speech patterns used in this scenario:
    Context: "${scenario.context}"
    Script example: "${scenario.script[0]?.text || ''}"
    
    Provide linguistic analysis in strictly valid JSON with keys:
    "languageName", "dialectOrEra", "keyVocabulary" (array of objects with "term", "meaning", "root"), "phoneticNotes".
    No markdown blocks.
  `;
...
export async function fetchSourceAnalysis(scenario: HistoricalScenario): Promise<any> {
  const sourcesList = scenario.sources.map(s => s.title).join(", ");
  const prompt = `
    Analyze the historical grounding and sources for this scenario:
    Context: "${scenario.context}"
    Sources: "${sourcesList}"
    
    Provide analysis in strictly valid JSON with keys:
    "credibilityScore" (number 1-100), "primarySourcesSummary", "historicalContextNuance".
    No markdown blocks.
  `;
